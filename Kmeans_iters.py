...
for j in range(0,30,10):
    print("Iteration: "+str(j/10 + 1))
    
    for i in range(0,1,1):

        Data = pd.read_csv("dump.Al.config", sep=" ", skiprows=9, index_col = False, names=['Id','Type','Pos_x','Pos_y','Pos_z','Sxx', 'Syy', 'Szz', 'Sxy', 'Sxz', 'Syz','Svm','Epot','Cna','Vol','Faces'] )
        ...
        TrueLabels = Data['Cna']
        Features = Data.drop(columns = ['Id','Type','Pos_x','Pos_y','Pos_z', 'Sxx', 'Syy', 'Szz', 'Sxy', 'Sxz', 'Syz','Cna','Faces'])
        ...
        Matriz.append(confusion_matrix(TrueLabels, Features['Cluster']))


# %%Confusion Matrix
mat0 = np.array(Matriz[0])
mat1 = np.array(Matriz[1])
mat2 = np.array(Matriz[2])
...
snsplot.figure.savefig("Matrices.pdf", bbox_inches="tight")
